...
class AbsenceReCordService extends Service
{
    ...
    public function punchTimeCardList($year, $month, $all = false)
    {
        ...
        foreach ($store_data as $key => $value) {
            ...
            $temp['list'] = [];
            $user_ids = User::where('store_id', '=', $value['id'])
                ->where('flag' . '=', '1')
    ...
    public function punchTimeCardList2($year, $month, $all = false)
    {
        ...
        foreach ($store_data as $key => $value) {
            ...
            foreach ($shedule_date_time as $key2 => $value2) {
                ...
                $user_id = [];
                // 查所有值班表id
                $sd_user_id = User::select('id', 'name')->where('store_id', '=', $value['id'])->where('flag', '=', '1')->get()->pluck('id')->toArray();
                $sd_user_id2 = User::select('id', 'name')->where('flag', '=', '1')->whereRaw("JSON_CONTAINS(support_store_id, '" . $value['id'] . "' )")->get()->pluck('id')->toArray();
                ...
                foreach ($user_ids as $key3 => $val3) {
                    ...
                    if (empty($temp_arr[$val3])) {
                        $user_data = User::select('id', 'name')->where('id', '=', $val3)->where('flag', '=', '1')->first();
                ...
                if (!empty($temp_list[$val2['day']])) {
                    foreach ($val2['staff'] as $key3 => $val3) {
                        if (!empty($temp_list[$val2['day']]['staff'][$val3['id']])) {
                            if (isset($val3['in']) && isset($val3['out'])) continue;
