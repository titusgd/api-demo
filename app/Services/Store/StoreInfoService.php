...
class StoreInfoService extends Service
{
    ...
    public function runValidate($method)
    {
        switch ($method) {
            case 'store':
                ...
                (!empty($this->request['shopHours']['week'])) && $rules['shopHours.week'] = 'string';
                (!empty($this->request['shopHours']['time'])) && $rules['shopHours.time'] = 'string';
                (!empty($this->request['map'])) && $rules['map'] = 'string';
                (!empty($this->request['image'])) && $rules['image'] = 'string';
    ...
    public function store()
    {
        ...
        // 整理調整資料
        $input_data = collect($this->request->all());

        $input_data
            ->put('use_flag', $this->request['use'])
            ->put('floor', implode(",", $this->request['floor']["f"]))
            ->put('basement', implode(",", $this->request['floor']["b"]))
            ->put('show_web', $this->request['showToWebsite'])
            ->put('week', $this->request['shopHours']["week"])
            ->put('time', $this->request['shopHours']["time"])
        ...
        if (!empty($this->request->image)) {
            $image_source = $this->request->image;
