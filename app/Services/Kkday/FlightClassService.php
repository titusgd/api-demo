...
// models
use App\Models\Kkday\FlightClass;
// methods
use App\Services\Kkday\KkdayService;
use Illuminate\Http\Request;
use App\Traits\RulesTrait;

class FlightClassService extends KkdayService
{
    use RulesTrait;
    ...
    public function store()
    {
        ...
        FlightClass::create($this->request->toArray());
    ...
    {
        if (!empty($this->response)) return $this;
        FlightClass::where('id', '=', $this->dataId)
    ...
    {
        if (!empty($this->response)) return $this;
        FlightClass::where('id', '=', $this->dataId)->delete();
        $this->response = KkdayService::response('00', 'ok');
        return $this;
    }
    ...
    {
        $data = KkdayService::loadJsonData(base_path() . '/app/Services/Kkday/FlightClass.json');
