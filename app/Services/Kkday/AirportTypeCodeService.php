...
// models
use App\Models\Kkday\AirportTypeCode;
// methods
...
class AirportTypeCodeService extends KkdayService
{
    use RulesTrait;
    ...
    public function __construct($request, $dataId = null)
    {
        ...
        $this->request
            ->put('description_ch', $request->get('descriptionCh', ''))
            ->put('description_en', $request->get('descriptionEn', ''));
    ...
    public function store()
    {
        ...
        AirportTypeCode::create($this->request->toArray());
    ...
    {
        if (!empty($this->response)) return $this;
        AirportTypeCode::where('id', '=', $this->dataId)
    ...
    {
        if (!empty($this->response)) return $this;
        AirportTypeCode::where('id', '=', $this->dataId)->delete();
        $this->response = KkdayService::response('00', 'ok');
        return $this;
    }
    ...
    {
        $data = KkdayService::loadJsonData(base_path() . '/app/Services/Kkday/AirportTypeCode.json');
        $now = date('Y-m-d H:i:s');
        foreach ($data as $key => $value) {
            $data[$key]['description_en'] = $value['description'];
            $data[$key]['created_at'] = $now;
            $data[$key]['updated_at'] = $now;
            unset($data[$key]['description']);
        }
        AirportTypeCode::insert($data);
        $this->response = KkdayService::response('00', 'ok');
