...
class ReceivableService extends Service
{

    use ReviewTrait;

    public $Transfer,$TransferItem,$Review;
    ...
    {      
        try {
            ...
            if ( isset($req->date->start) ) {
                    array_push($where,['receipts.created_at', '>=', $req->date->start . " 00:00:00"]);
            }
            if ( isset($req->date->end) ) {
                    array_push($where,['receipts.created_at', '<=', $req->date->end . " 23:59:59"]);
            }

            // 查詢資料
            $res = ReceiptItem::select(
                "receipts.code",
                "receipt_items.id",
                "receipt_items.receipt_id",
                "summary",
                "receive_date as receiveDate",
                DB::raw("(select code from transfer_vouchers where id = receipt_items.transfer_voucher_id) as transferVoucher"),
            ...
            ->where(function ($query)  {
                    $query->where("accounting_subject_id",46);                
                    $query->orwhere(DB::raw("(select subject_id from accounting_subjects where id = accounting_subject_id)"),"=",46);
            })
            ->with([
                'subject',
                'status'
    ...
    public function create($req)
    {
        ...
        try {
            ...
            $insert_data = ReceiptItem::select(
            ...
            // 更新id回付款單，代表已銷帳
            ReceiptItem::whereIn('id',$req['select'])
            ->update(
                [
